...
# Mat??rn covariance function (eq.2.3) 
...
# For other variables, mean accross duplicates is computed
...
### standard spde formulation, i.e. obtain C_0, G_1 and G_2 (eq. 2.4)
...
parameters3 <- list(
  ...
  log_tau   = 1.45,                    # tau from eq.2.4 (on log scale)
  log_kappa = -2.77,                   # kappa from eq.2.4 (on log scale)
...
f <- function(parms) {
   ...
   # Transform back parameters' starting values
    ...
    # Compute  the precision matrix Q (eq. 2.4)
